...
import { extractMatrixResultList, handleMatrixResponse, isVerbose } from "./matrix-api.ts";
  ...
  if (idsStr != null && idsStr !== "") {
    ...
    try {
      ...
      if (raw.startsWith("[") && raw.endsWith("]")) {
        ...
        const parts = inner ? inner.split(",").map((s) => parseInt(s.trim(), 10)).filter((n) => !Number.isNaN(n)) : [];
...
export async function queryMatrixProjects(
  ...
  if (args.projectName != null && args.projectName !== "") body.projectName = args.projectName;
  ...
  const rows = list.map(formatRow).filter((r): r is { id: number; name: string } => r != null);
  if (isVerbose()) {
    console.log("\ndata.result 简化结果：");
    for (const r of rows) {
