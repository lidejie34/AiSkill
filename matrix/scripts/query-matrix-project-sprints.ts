...
import { handleMatrixResponse, extractMatrixResultList, resolveProjectId, resolveItemStatusValueByName, isVerbose } from "./matrix-api.ts";
...
export function getSprintsArgsFromArgv(): SprintsListArgs {
  ...
  if (statusStr != null && statusStr !== "") {
    try {
      ...
      if (!Array.isArray(arr)) {
        ...
        statusCondition = arr.map((x) => {
          if (typeof x === "number" && !Number.isNaN(x)) return x;
          if (typeof x === "string" && x.trim() !== "") return x.trim();
          const n = Number(x);
          return Number.isNaN(n) ? (typeof x === "string" ? (x as string).trim() : undefined) : n;
        }).filter((x): x is number | string => x !== undefined && x !== "") as (number | string)[];
        if (statusCondition.length === 0) statusCondition = undefined;
      }
    } catch {
      const parts = statusStr.split(",").map((s) => s.trim()).filter((s) => s !== "");
      if (parts.length > 0) {
  ...
  return { page: pageVal, pageSize: pageSizeVal, projectId: projectIdStr ?? undefined, sprintName: sprintName ?? undefined, statusCondition };
}

/** 从单条记录中取 id、projectName（兼容 id/projectId、projectName/name） */
function formatRow(item: unknown): { id: number; name: string, projectName: string } | null {
...
export async function queryMatrixProjectSprints(
  ...
  if (args.projectId != null) body.projectId = await resolveProjectId(accessToken, args.projectId);
  ...
  const rows = list.map(formatRow).filter((r): r is { id: number; name: string; projectName: string } => r != null);
  if (isVerbose()) {
    console.log("\ndata.result 简化结果：");
    for (const r of rows) {
      console.log(`  id: ${r.id}, name: ${r.name}, projectName: ${r.projectName}`);
    }
  }
}
