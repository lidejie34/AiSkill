...
export type MatrixResponse = { code?: string; message?: string; [k: string]: unknown };
...
const REQUIREMENT_SEARCH_BASE = "https://matrix.17usoft.com/api/pool/project/new/search";
/** custom-status/items 与 queryMatrixItemsStatus 一致 */
const CUSTOM_STATUS_ITEMS_URL = "https://matrix.17usoft.com/api/project/custom-status/items";
 ...
 */
export function extractMatrixResultList(data: Record<string, unknown>): unknown[] {
  ...
  if (d && d.data != null && typeof d.data === "object" && !Array.isArray(d.data)) {
 ...
 */
export async function handleMatrixResponse(res: Response, apiName: string): Promise<Record<string, unknown>> {
  if (!res.ok) {
...
export async function fetchMatrixJson(
  ...
  if (isVerbose()) {
    ...
    if (init.body != null) console.log("[verbose] fetch body:", typeof init.body === "string" ? init.body : "[RequestInit]");
  }
  const res = await fetch(url, {
    ...init,
    headers: { ...defaultHeaders(accessToken), ...(init.headers as Record<string, string>) },
    signal: AbortSignal.timeout(15000),
  });
  if (!res.ok) throw new Error(`请求失败 HTTP ${res.status}`);
  const data = (await res.json()) as Record<string, unknown>;
  if (data?.code !== "0000") throw new Error(`接口返回 code=${data?.code ?? "(无)"}`);
  return data;
}
...
export async function resolveProjectId(
  ...
  if (exact.length > 1) {
    throw new Error(`按项目名称「${trimmed}」精确匹配到多条，请使用数字 projectId 或通过 queryMatrixProjects 确认`);
  }
  if (exactOnly) {
  ...
  if (projects.length === 0) {
    throw new Error(`未找到与项目名「${trimmed}」匹配的项目，请使用 queryMatrixProjects 核对或直接使用数字 projectId`);
  }
...
export async function resolveProjectIds(
  ...
  return Promise.all((projectIds as string[]).map((name) => resolveProjectId(accessToken, name)));
}
...
export async function resolveSprintId(
  ...
  if (exact.length === 1) return exact[0]!.id;
  if (exact.length > 1) {
    throw new Error(`按迭代名称「${trimmed}」精确匹配到多条，请使用数字 sprintId 或通过 queryMatrixProjectSprints 确认`);
  }
  if (exactOnly) {
    if (sprints.length === 0) {
      throw new Error(
  ...
  if (sprints.length === 0) {
    throw new Error(`项目 ${projectId} 下未找到与迭代名「${trimmed}」匹配的迭代，请使用 queryMatrixProjectSprints 核对或直接使用数字 sprintId`);
  }
...
export async function resolveSprintIds(
  ...
  if (allNums.every((n) => !Number.isNaN(n))) return allNums as number[];
  return Promise.all((sIds as string[]).map((name) => resolveSprintId(accessToken, projectId, name)));
}
...
export async function resolveRequirementId(
  ...
  if (exact.length === 1) return exact[0]!.id;
  if (exact.length > 1) {
    throw new Error(`按需求名称「${trimmed}」查询到多条，请使用数字 requirementId 或通过 queryMatrixProjectRequirements 确认`);
  }
  if (reqs.length === 0) {
    throw new Error(`项目 ${projectId} 下未找到与需求名「${trimmed}」匹配的需求，请使用 queryMatrixProjectRequirements 核对或直接使用数字 requirementId`);
  }
...
export async function resolveRequirementIds(
  ...
  if (allNums.every((n) => !Number.isNaN(n))) return allNums as number[];
  return Promise.all((requirementIds as string[]).map((name) => resolveRequirementId(accessToken, projectId, name)));
}
...
function getMapFromItemsData(data: Record<string, unknown>, field: ItemStatusField): Record<string, string> | null {
  const d = data.data as Record<string, unknown> | undefined;
  const raw =
...
export async function resolveItemStatusValueByName(
  ...
  if (trimmed === "") throw new Error(`resolveItemStatusValueByName 的 name 不能为空`);

  if (isVerbose()) console.log("[verbose] fetch url:", CUSTOM_STATUS_ITEMS_URL, "(GET)");
  ...
  const data = (await res.json()) as Record<string, unknown>;
  if (data?.code !== "0000") throw new Error(`custom-status/items 返回 code=${data?.code ?? "(无)"}`);

  const map = getMapFromItemsData(data, field);
  if (!map) {
    throw new Error(`custom-status/items 中未找到字段「${field}」，请使用 queryMatrixItemsStatus 查看`);
  }
  ...
  if (matches.length === 0) {
    const byInclude = Object.entries(map).filter(
      ([_, val]) => val.includes(trimmed) || trimmed.includes(val) || val.toLowerCase().includes(lower)
    );
    ...
    if (byInclude.length > 1) {
      throw new Error(`在 ${field} 中按名称「${name}」匹配到多条：${byInclude.map(([k, v]) => `${k}:${v}`).join(", ")}`);
    }
    throw new Error(`在 ${field} 中未找到与「${name}」对应的项，请使用 queryMatrixItemsStatus 查看`);
  }
  if (matches.length > 1) {
    throw new Error(`在 ${field} 中按名称「${name}」匹配到多条：${matches.map((m) => m.key + ":" + m.name).join(", ")}`);
  }
