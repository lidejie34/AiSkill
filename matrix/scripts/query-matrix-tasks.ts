...
function getTaskTypeList(data: Record<string, unknown>): { code: number; name: string }[] {
  const raw =
    (data.taskTypeMap as Record<string, string> | undefined) ??
    (data.data as Record<string, unknown> | undefined)?.taskTypeMap as Record<string, string> | undefined;
  ...
  return Object.entries(raw)
    ...
    .filter((x): x is { code: number; name: string } => x.name !== "" && typeof x.code === "number");
}

/** 将 type 解析为任务类型码：整型直接返回，字符串从 taskTypeMap 按 name 解析 */
export async function resolveTaskTypeCode(accessToken: string, typeStr: string): Promise<number> {
  const trimmed = typeStr.trim();
  const asNum = parseInt(trimmed, 10);
  if (!Number.isNaN(asNum) && String(asNum) === trimmed) return asNum;

  const data = (await getItemsStatusData(accessToken)) as Record<string, unknown>;
  const list = getTaskTypeList(data);
  const lower = trimmed.toLowerCase();
  const exact = list.find((x) => x.name === trimmed || x.name.toLowerCase() === lower);
  if (exact) return exact.code;
  const includes = list.find((x) => x.name.includes(trimmed) || trimmed.includes(x.name) || x.name.toLowerCase().includes(lower) || lower.includes(x.name.toLowerCase()));
  if (includes) return includes.code;
  console.error("错误: 未在 custom-status/items 的 taskTypeMap 中按 name 找到与「" + trimmed + "」匹配的任务类型，请使用数字或 queryMatrixItemsStatus 查看");
  process.exit(1);
  return 0 as never;
}
...
export type QueryTasksArgsFromArgv = Omit<QueryTasksArgs, "type"> & { typeStr?: string };
...
/** 从单条记录中打印简化结果 */
function formatRow(item: unknown): { id: number; name: string, 
  projectName: string,sprintName: string ,requirementName: string} | null {
  ...
  return { id: idNum, name: name || "(无名称)", projectName ,sprintName,requirementName};
}
...
export async function queryMatrixTasks(accessToken: string, args: QueryTasksArgsFromArgv): Promise<void> {
  ...
  if (args.sIds != null && args.sIds.length > 0) {
    if (typeof args.sIds[0] === "string") {
      if (resolvedProjectIds == null || resolvedProjectIds.length === 0) {
        console.error("错误: sIds 为名称时需同时指定 projectIds（迭代名按项目解析）");
        process.exit(1);
      }
      resolvedSIds = await resolveSprintIds(accessToken, resolvedProjectIds[0]!, args.sIds);
    } else {
  ...
  const resolved: QueryTasksArgs = { ...args, type: typeVal, projectIds: resolvedProjectIds, sIds: resolvedSIds, statusCondition: resolvedStatusCondition };
  const url = `${TASK_LIST_BASE}/${resolved.page}/${resolved.pageSize}`;
  const body: Record<string, unknown> = {};
  if (resolved.processor != null && resolved.processor !== "") body.processor = resolved.processor;
  if (resolved.projectIds != null && resolved.projectIds.length > 0) body.projectIds = resolved.projectIds;
  if (resolved.statusCondition != null && resolved.statusCondition.length > 0) body.statusCondition = resolved.statusCondition;
  if (resolved.sIds != null && resolved.sIds.length > 0) body.sIds = resolved.sIds;
  if (resolved.creator != null && resolved.creator !== "") body.creator = resolved.creator;
  if (resolved.cCreateTimeS != null && resolved.cCreateTimeS !== "") body.cCreateTimeS = resolved.cCreateTimeS;
  if (resolved.cCreateTimeE != null && resolved.cCreateTimeE !== "") body.cCreateTimeE = resolved.cCreateTimeE;
  if (resolved.cExpectFinishTimeE != null && resolved.cExpectFinishTimeE !== "") body.cExpectFinishTimeE = resolved.cExpectFinishTimeE;
  if (resolved.cExpectFinishTimeS != null && resolved.cExpectFinishTimeS !== "") body.cExpectFinishTimeS = resolved.cExpectFinishTimeS;
  if (resolved.cExpectQaEndTimeE != null && resolved.cExpectQaEndTimeE !== "") body.cExpectQaEndTimeE = resolved.cExpectQaEndTimeE;
  if (resolved.cExpectQaEndTimeS != null && resolved.cExpectQaEndTimeS !== "") body.cExpectQaEndTimeS = resolved.cExpectQaEndTimeS;
  if (resolved.cRealEndTimeE != null && resolved.cRealEndTimeE !== "") body.cRealEndTimeE = resolved.cRealEndTimeE;
  if (resolved.cRealEndTimeS != null && resolved.cRealEndTimeS !== "") body.cRealEndTimeS = resolved.cRealEndTimeS;
  if (resolved.cTestEndTimeE != null && resolved.cTestEndTimeE !== "") body.cTestEndTimeE = resolved.cTestEndTimeE;
  if (resolved.cTestEndTimeS != null && resolved.cTestEndTimeS !== "") body.cTestEndTimeS = resolved.cTestEndTimeS;
  if (resolved.cExpectDeployTimeE != null && resolved.cExpectDeployTimeE !== "") body.cExpectDeployTimeE = resolved.cExpectDeployTimeE;
  if (resolved.cExpectDeployTimeS != null && resolved.cExpectDeployTimeS !== "") body.cExpectDeployTimeS = resolved.cExpectDeployTimeS;
  if (resolved.cRealDeployTimeE != null && resolved.cRealDeployTimeE !== "") body.cRealDeployTimeE = resolved.cRealDeployTimeE;
  if (resolved.cRealDeployTimeS != null && resolved.cRealDeployTimeS !== "") body.cRealDeployTimeS = resolved.cRealDeployTimeS;
  ...
  const rows = list.map(formatRow).filter((r): r is { id: number; name: string; projectName: string; sprintName: string; requirementName: string } => r != null);
  if (isVerbose()) {
    console.log("\ndata.result 简化结果：");
    for (const r of rows) {
      console.log(`  id: ${r.id}, name: ${r.name}, projectName: ${r.projectName}, sprintName: ${r.sprintName}`);
    }
  }
}
