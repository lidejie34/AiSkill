...
import { handleMatrixResponse, resolveProjectId, resolveSprintIds, resolveItemStatusValueByName, extractMatrixResultList, isVerbose } from "./matrix-api.ts";

const REQUIREMENT_LIST_BASE = "https://matrix.17usoft.com/api/pool/project/new/search";
...
export function getQueryProjectRequirementsArgsFromArgv(): QueryProjectRequirementsArgs {
  ...
  if (projectIdStr == null || projectIdStr === "") {
    console.error("错误: queryMatrixProjectRequirements 需要 --projectId=项目id或名称 可通过 queryMatrixProjects 按名称查询）");
    process.exit(1);
  }
  ...
  if (priorityStr != null && priorityStr !== "") {
    ...
    priority = !Number.isNaN(n) && String(n) === priorityStr.trim() ? n : priorityStr.trim();
  }
  ...
  if (rpTypeStr != null && rpTypeStr !== "") {
    ...
    rpType = !Number.isNaN(n) && String(n) === rpTypeStr.trim() ? n : rpTypeStr.trim();
  }

  const sIdsRaw = getArgFromArgv("sIds");
  const sIds = sIdsRaw != null && sIdsRaw !== "" ? parseIdOrNameArray(sIdsRaw) : undefined;
...
function formatRow(item: unknown): { id: number; name: string, projectName: string,sprintName: string } | null {
...
export async function queryMatrixProjectRequirements(
  ...
  if (typeof args.priority === "string" && args.priority !== "") {
    priorityVal = await resolveItemStatusValueByName(accessToken, "reqPriority", args.priority) as number;
  }

  let rpTypeVal: number | string | undefined = args.rpType;
  if (typeof args.rpType === "string" && args.rpType !== "") {
    rpTypeVal = await resolveItemStatusValueByName(accessToken, "reqType", args.rpType) as number | string;
  }
  ...
  if (args.requireName != null && args.requireName !== "") body.requireName = args.requireName;
  if (args.id != null) body.id = args.id;
  if (priorityVal != null) body.priority = priorityVal;
  if (args.demander != null && args.demander !== "") body.demander = args.demander;
  if (args.proposer != null && args.proposer !== "") body.proposer = args.proposer;
  ...
  const rows = list.map(formatRow).filter((r): r is { id: number; name: string; projectName: string; sprintName: string } => r != null);
  if (isVerbose()) {
    console.log("\ndata.result 简化结果：");
    for (const r of rows) {
