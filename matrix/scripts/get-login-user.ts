...
export const GET_LOGIN_USER_URL = "https://matrix.17usoft.com/api/user/getLoginUser";
 ...
 */
export async function fetchLoginUserNameJob(accessToken: string): Promise<string> {
  const data = await fetchMatrixJson(accessToken, GET_LOGIN_USER_URL, { method: "GET" });
  const d = (data.data as Record<string, unknown> | undefined) ?? (data as Record<string, unknown>);
  ...
  const name = pickStr(u, ["userName", "name", "realName", "nickName", "userNick"]);
  const job = pickStr(u, ["newWorkId", "workId", "jobNumber", "workNo", "empNo", "userCode", "code", "jobNo"]);
  if (name !== "" && job !== "") return `${name}-${job}`;
  const combined = pickStr(u, ["userNameJob", "displayName", "label"]).trim() || pickStr(d, ["userNameJob", "displayName", "label"]);
