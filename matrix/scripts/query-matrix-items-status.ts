...
import { handleMatrixResponse, type MatrixResponse, isVerbose } from "./matrix-api.ts";

/** 自定义状态与类型枚举接口：GET /api/project/custom-status/items */
export const CUSTOM_STATUS_ITEMS_URL = "https://matrix.17usoft.com/api/project/custom-status/items";
...
export async function getItemsStatusData(accessToken: string): Promise<MatrixResponse> {
...
export async function queryMatrixItemsStatus(accessToken: string): Promise<void> {
  if (isVerbose()) console.log("[verbose] fetch url:", CUSTOM_STATUS_ITEMS_URL, "(GET)");
  const res = await fetch(CUSTOM_STATUS_ITEMS_URL, {
    method: "GET",
    headers: DEFAULT_HEADERS(accessToken),
