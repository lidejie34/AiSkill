...
export function parseIdOrNameArray(s: string | null): number[] | string[] | undefined {
...
export function parseCodeOrNameArray(s: string | null): number[] | string[] | undefined {
  if (s == null || s === "") return undefined;
  const raw = s.trim();
  const parts = raw.split(",").map((p) => p.trim()).filter(Boolean);
  if (parts.length === 0) return undefined;
  ...
  for (const v of items) {
    if (typeof v === "number") {
      ...
      if (trimmed !== "" && !Number.isNaN(asNum) && String(asNum) === trimmed) {
        ...
        resolved.push(typeof code === "number" ? code : parseInt(String(code), 10));
      }
    }
  }
  return resolved;
