 ...
 * 用法：matrix -- createMatrixTask --projectId=项目id或名称 --taskName=任务名称 [--requirementId=] [--type=] [--sprintId=] [--priority=] [--description=] ...
...
export function buildCreateTaskBody(
  ...
  const body: Record<string, unknown> = {
    ...
    type,
    priority,
    ifTest: args.ifTest != null ? args.ifTest : 0,
    expectRdHours: args.expectRdHours != null ? args.expectRdHours : 0,
  };
  if (requirementId != null) body.requirementId = requirementId;
  if (sprintId != null) body.sprintId = sprintId;
  if (args.expectBeginTime != null && args.expectBeginTime !== "") body.expectBeginTime = args.expectBeginTime;
  if (args.expectEndTime != null && args.expectEndTime !== "") body.expectEndTime = args.expectEndTime;
  if (args.description != null && args.description !== "") body.description = args.description;
  return body;
}
...
export function getCreateTaskArgsFromArgv(): CreateTaskArgs {
  ...
  if (projectId == null || projectId === "") {
    console.error("错误: createMatrixTask 需要 --projectId=项目id（或项目名称，将通过 queryMatrixProjects 解析）");
    process.exit(1);
  }
...
export async function createMatrixTask(accessToken: string, args: CreateTaskArgs): Promise<void> {
  const projectId = await runResolve(() => resolveProjectId(accessToken, args.projectId));

  let requirementId: number | undefined;
  if (args.requirementId != null && args.requirementId !== "") {
    const reqName = args.requirementId;
    requirementId = await runResolve(() => resolveRequirementId(accessToken, projectId, reqName));
  }
  ...
  if (args.priority != null && args.priority !== "") {
    ...
    const v = await runResolve(() => resolveItemStatusValueByName(accessToken, "taskPriority", p));
    priority = typeof v === "number" ? v : parseInt(String(v), 10);
  } else {
    const v = await runResolve(() => resolveItemStatusValueByName(accessToken, "taskPriority", DEFAULT_PRIORITY_NAME));
    priority = typeof v === "number" ? v : parseInt(String(v), 10);
  }

  const sprintId =
    args.sprintId != null && args.sprintId !== ""
      ? await runResolve(() => resolveSprintId(accessToken, projectId, args.sprintId!))
      : undefined;
  const body = buildCreateTaskBody(args, projectId, type, priority, requirementId, sprintId);
