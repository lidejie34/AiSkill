...
const BUG_SELECT_PAGE_BASE = "https://matrix.17usoft.com/api/bug/selectAllBugWithPage";
...
export function getQueryBugsArgsFromArgv(options?: GetQueryBugsArgsFromArgvOptions): QueryBugsArgsFromArgv {
  const rel = getArgFromArgv("relatedToMe");
  if (rel != null && rel !== "") {
    const off = rel === "0" || rel.toLowerCase() === "false" || rel.toLowerCase() === "no";
    if (off) {
      console.error("错误: queryMatrixBugs 仅支持查询与当前登录用户相关的缺陷，不能关闭（请勿使用 --relatedToMe=0）");
      process.exit(1);
    }
  }
  ...
  return {
    ...
    startWaitVerificationTime: getArgFromArgv("startWaitVerificationTime") ?? undefined,
    startRealResolveTime: getArgFromArgv("startRealResolveTime") ?? undefined,
    assigner: options?.omitPersonOverrides ? undefined : trimArg(getArgFromArgv("assigner")),
    creator: options?.omitPersonOverrides ? undefined : trimArg(getArgFromArgv("creator")),
    owner: options?.omitPersonOverrides ? undefined : trimArg(getArgFromArgv("owner")),
  };
}
...
function formatBugRow(item: unknown): {
  ...
  return { id: idNum, bugName: bugName || "(无名称)", projectName, sprintName, creator, assigner, owner };
}
...
async function resolveBugLevels(
  ...
  const resolved = await Promise.all(
    items.map(async (v) => {
      ...
      if (trimmed !== "" && !Number.isNaN(asNum) && String(asNum) === trimmed) return asNum;
      const code = await resolveItemStatusValueByName(accessToken, "bugLevel", trimmed);
...
async function resolveBugPriorityNumber(accessToken: string, raw: string | undefined): Promise<number | undefined> {
  if (raw == null || raw === "") return undefined;
  const trimmed = raw.trim();
  const asNum = parseInt(trimmed, 10);
  if (!Number.isNaN(asNum) && String(asNum) === trimmed) return asNum;
  const code = await resolveItemStatusValueByName(accessToken, "bugPriority", trimmed);
  return typeof code === "number" ? code : parseInt(String(code), 10);
}
...
function assertPersonOverrideIncludesMe(field: PersonField, raw: string, currentUserNameJob: string): void {
  const parts = raw
    .split(",")
    .map((p) => p.trim())
    .filter(Boolean);
  ...
  console.error(`错误: --${field} 须包含当前登录用户「${currentUserNameJob}」，已传：${raw}`);
  process.exit(1);
}

function assertAllPersonOverridesIncludeMe(args: QueryBugsArgs, currentUserNameJob: string): void {
  if (args.assigner) assertPersonOverrideIncludesMe("assigner", args.assigner, currentUserNameJob);
  if (args.creator) assertPersonOverrideIncludesMe("creator", args.creator, currentUserNameJob);
  if (args.owner) assertPersonOverrideIncludesMe("owner", args.owner, currentUserNameJob);
}

/** 该维度写入接口的 assigner/creator/owner 字符串：有覆盖用覆盖（已校验含本人），否则用本人 */
function personValueForField(field: PersonField, args: QueryBugsArgs, currentUserNameJob: string): string {
  const raw = field === "assigner" ? args.assigner : field === "creator" ? args.creator : args.owner;
...
async function buildBugQueryBody(
  ...
  if (args.bSolvers != null && args.bSolvers.length > 0) body.bSolvers = args.bSolvers;
  if (resolvedLevels != null && resolvedLevels.length > 0) body.levels = resolvedLevels;
  if (priorityNum != null) body.priority = priorityNum;
  if (resolvedStatus != null && resolvedStatus.length > 0) body.statusCondition = resolvedStatus;
  if (resolvedProjectIds != null && resolvedProjectIds.length > 0) body.projectIds = resolvedProjectIds;
  if (resolvedSIds != null && resolvedSIds.length > 0) body.sIds = resolvedSIds;
  if (args.startCompletionTime) body.startCompletionTime = args.startCompletionTime;
  if (args.endCompletionTime) body.endCompletionTime = args.endCompletionTime;
  if (args.startWaitVerificationTime) body.startWaitVerificationTime = args.startWaitVerificationTime;
  if (args.startRealResolveTime) body.startRealResolveTime = args.startRealResolveTime;
...
function printBugRowsVerbose(list: unknown[], label?: string): void {
  const rows = list.map(formatBugRow).filter((r): r is NonNullable<typeof r> => r != null);
...
export async function fetchRelatedBugsForPick(
  ...
  if (args.priorityStr != null && args.priorityStr !== "") {
    priorityResolved = await resolveBugPriorityNumber(accessToken, args.priorityStr);
  }
  ...
  for (const field of dimensions) {
    ...
    const body = await buildBugQueryBody(accessToken, base, { field, value: personValue });
    const data = await postSelectBugs(accessToken, args.page, args.pageSize, body);
...
export async function queryMatrixBugs(accessToken: string, args: QueryBugsArgsFromArgv): Promise<void> {
  let nameJob: string;
  try {
