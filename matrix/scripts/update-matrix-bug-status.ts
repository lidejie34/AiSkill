...
import { handleMatrixResponse, resolveItemStatusValueByName, isVerbose } from "./matrix-api.ts";
import { getArgFromArgv } from "./parse-arg.ts";
import { fetchRelatedBugsForPick, getQueryBugsArgsFromArgv } from "./query-matrix-bugs.ts";
...
export function parseUpdateBugStatusArgv(): {
  ...
  if (statusRaw == null || statusRaw.trim() === "") {
    console.error("错误: updateMatrixBugStatus 需要 --status=缺陷状态（数字或状态名，见 queryMatrixItemsStatus 的 bugStatus）");
    process.exit(1);
  }
  if (assigner.trim() === "") {
    console.error("错误: updateMatrixBugStatus 需要 --assigner=下一阶段处理人（格式：姓名-工号）");
    process.exit(1);
  }
...
async function resolveStatusCode(accessToken: string, statusRaw: string): Promise<number> {
  const trimmed = statusRaw.trim();
  const asNum = parseInt(trimmed, 10);
  if (!Number.isNaN(asNum) && String(asNum) === trimmed) return asNum;
  const v = await resolveItemStatusValueByName(accessToken, "bugStatus", trimmed);
  return typeof v === "number" ? v : parseInt(String(v), 10);
}

function printCandidateBugs(rows: Awaited<ReturnType<typeof fetchRelatedBugsForPick>>): void {
  console.log("未指定 --bugId，以下为与当前登录用户相关的缺陷候选（queryMatrixBugs 三维度合并去重），请选定 id 后执行：");
  console.log("");
  for (const r of rows) {
    console.log(
  ...
  console.log("");
  console.error("请使用 --bugId=<上表中的 id> 并保留 --status= 与 --assigner= 再次执行 updateMatrixBugStatus。");
}

/** PUT /api/bug/:bugId/statusChange */
export async function updateMatrixBugStatus(accessToken: string, args: UpdateBugStatusArgs): Promise<void> {
 ...
 */
export async function runUpdateMatrixBugStatusFromArgv(accessToken: string): Promise<void> {
  ...
  if (parsed.bugId == null) {
    ...
    const queryBugsArgs = getQueryBugsArgsFromArgv({ omitPersonOverrides: true });
    const rows = await fetchRelatedBugsForPick(accessToken, queryBugsArgs);
    if (rows.length === 0) {
      console.error("未找到与我相关的缺陷，请先用 queryMatrixBugs 确认或使用 --bugId= 指定缺陷 id。");
      process.exit(1);
    }
  ...
  const status = await resolveStatusCode(accessToken, parsed.statusRaw).catch((e: Error) => {
