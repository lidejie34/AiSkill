...
export function writeCache(accessToken: string, url: string = MATRIX_URL): void {
  const map = readCacheMap();
...
export function pickAccessToken(cookies: CookieItem[]): string | null {
  const tokenCookies = cookies.filter((c) => c.name === "access_token" && c.value?.trim());
...
export async function checkAccessTokenAvailable(accessToken: string): Promise<boolean> {
