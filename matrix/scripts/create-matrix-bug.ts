...
async function resolveOptionalBugCode(
  ...
  const v = await runResolve(() => resolveItemStatusValueByName(accessToken, field, trimmed));
...
export function getCreateBugArgsFromArgv(): CreateBugArgs {
  ...
  if (assigner == null || assigner === "") {
    console.error("错误: createMatrixBug 需要 --assigner=缺陷处理人（格式：姓名-工号）");
    process.exit(1);
  }
  if (completionTime == null || completionTime === "") {
    console.error("错误: createMatrixBug 需要 --completionTime=期望修复时间（格式：yyyy-MM-dd HH:mm:ss）");
    process.exit(1);
  }
...
export async function createMatrixBug(accessToken: string, args: CreateBugArgs): Promise<void> {
  const projectId = await runResolve(() =>
    resolveProjectId(accessToken, args.projectId, { exactNameMatchOnly: true })
  );

  const description =
    args.description != null && args.description !== "" ? args.description : DEFAULT_BUG_DESCRIPTION;
  ...
  const sprintId = await runResolve(() =>
    resolveSprintId(accessToken, projectId, args.sprintId, { exactNameMatchOnly: true })
  );

  const type = await resolveOptionalBugCode(accessToken, "bugType", args.type);
  const level = await resolveOptionalBugCode(accessToken, "bugLevel", args.level);
  const priority = await resolveOptionalBugCode(accessToken, "bugPriority", args.priority);
