...
import { getSprintsArgsFromArgv, queryMatrixProjectSprints } from "./query-matrix-project-sprints.ts";
import { getProjectListArgsFromArgv, queryMatrixProjects } from "./query-matrix-projects.ts";
import { queryMatrixItemsStatus } from "./query-matrix-items-status.ts";
import { getCreateProjectArgsFromArgv, createMatrixProject } from "./create-matrix-project.ts";
import { getCreateSprintArgsFromArgv, createMatrixSprint } from "./create-matrix-sprint.ts";
import { getCreateTaskArgsFromArgv, createMatrixTask } from "./create-matrix-task.ts";
import { getCreateBugArgsFromArgv, createMatrixBug } from "./create-matrix-bug.ts";
import { getLoginUser } from "./get-login-user.ts";
import { getQueryTasksArgsFromArgv, queryMatrixTasks } from "./query-matrix-tasks.ts";
import { getQueryBugsArgsFromArgv, queryMatrixBugs } from "./query-matrix-bugs.ts";
import { runUpdateMatrixBugStatusFromArgv } from "./update-matrix-bug-status.ts";
import { getQueryProjectRequirementsArgsFromArgv, queryMatrixProjectRequirements } from "./query-matrix-project-requirements.ts";
...
function stripVerbose(): void {
  ...
  let i = 0;
  while (i < argv.length) {
    if (argv[i] === "--verbose" || argv[i] === "-v" || argv[i]!.startsWith("--verbose=")) {
...
function getSubcommand(): string | null {
  ...
  if (sub === "auth") {
    const authAction = args[0] === "--" ? args[2] : args[1] === "matrix" ? args[2] : args[1];
  ...
  if (sub === "queryMatrixProjectRequirements") return "queryMatrixProjectRequirements";
...
function launchChrome(): import("child_process").ChildProcess | null {
  ...
  const userDataDir = process.env["CHROME_USER_DATA_DIR"] ?? "/tmp/matrix-cdp-profile";
...
function findPageWsUrl(targets: { type?: string; webSocketDebuggerUrl?: string }[]): string | null {
  for (const t of targets) {
    if (t?.webSocketDebuggerUrl && (t.type === "page" || t.type === "tab")) return t.webSocketDebuggerUrl;
  }
  return null;
}
...
async function getPageWsUrl(): Promise<string> {
  ...
  while (Date.now() < deadline) {
    try {
      const targets = await httpGet<{ type?: string; webSocketDebuggerUrl?: string }[]>(
...
function connectCDP(wsUrl: string): Promise<{
  ...
  return new Promise((resolve, reject) => {
    ...
    let nextId = 1;
    const pending = new Map<number, { resolve: (r: CDPMessage["result"]) => void; reject: (e: Error) => void }>();

    ws.addEventListener("message", (ev: MessageEvent) => {
      const data: CDPMessage = JSON.parse(typeof ev.data === "string" ? ev.data : (ev.data as Buffer).toString());
    ...
    ws.addEventListener("error", (e) => reject(e instanceof Error ? e : new Error(String(e))));
  });
}

type CDPClient = Awaited<ReturnType<typeof connectCDP>>;

async function getCookiesFromCDP(client: CDPClient, urls?: string[]): Promise<CookieItem[]> {
...
async function connectAndNavigateToMatrix(chromeProc: import("child_process").ChildProcess | null): Promise<CDPClient> {
...
async function waitForAccessToken(
  ...
  console.log(`请在浏览器中完成登录，脚本将等待 access_token（超时 ${WAIT_TIMEOUT_MS / 1000} 秒）...`);
  const deadline = Date.now() + WAIT_TIMEOUT_MS;
  let accessToken: string | null = null;

  while (Date.now() < deadline) {
    const matrixCookies = await getCookiesFromCDP(client, [MATRIX_URL, `https://${MATRIX_DOMAIN}`]);
    accessToken = pickAccessToken(matrixCookies);
    let otherCookies: CookieItem[] = [];
    if (!accessToken) {
      otherCookies = await getCookiesFromCDP(client, ["https://.17usoft.com/", "https://17usoft.com/"]);
      accessToken = pickAccessToken(otherCookies);
    }
    if (process.env["DEBUG_COOKIE"] === "1") {
      const all = [...matrixCookies];
      for (const c of otherCookies) {
        if (!all.some((x) => x.name === c.name && x.domain === c.domain)) all.push(c);
      }
      console.error("[调试] 当前 Cookie 数量:", all.length);
      for (const c of all) {
        const preview = (c.value ?? "").slice(0, 20) + ((c.value?.length ?? 0) > 20 ? "..." : "");
        console.error(`  name=${JSON.stringify(c.name)} domain=${JSON.stringify(c.domain)} value_preview=${JSON.stringify(preview)}`);
      }
    }
    if (accessToken) {
  ...
  client.close();
  if (chromeProc && !useExisting) try { chromeProc.kill(); } catch {}
...
const TCODE_UPGRADE_URL = "https://tws.17usoft.com/copilot/skill/detail?slug=matrix&space=tongcheng";

/** 通过 tcode auth token 获取 accessToken */
function getAccessTokenFromTcode(): string {
  try {
    const stdout = execSync("tcode auth token", { encoding: "utf-8", timeout: 15000 }).trim();
    if (!stdout) {
      console.error("错误: tcode auth token 返回为空。请升级 tcode 版本: " + TCODE_UPGRADE_URL);
      process.exit(1);
    }
    return stdout;
  } catch (e) {
    console.error("错误: 执行 tcode auth token 失败，请升级 tcode 版本: " + TCODE_UPGRADE_URL);
    console.error(e instanceof Error ? e.message : e);
    process.exit(1);
  }
}

/** 获取 token（缓存或 tcode 登录）并执行 fn */
async function runWithToken(fn: (accessToken: string) => Promise<void>): Promise<void> {
  ...
  if (cachedToken) {
    ...
    deleteCache();
    console.log("缓存 token 已失效，将使用 tcode 登录...");
  }
  const accessToken = getAccessTokenFromTcode();
...
async function main(): Promise<void> {
  ...
  console.error("  matrix auth login                          # 使用 tcode 登录并缓存 accessToken");
  console.error("  matrix auth logout                         # 清理所有本地缓存");
  console.error("  matrix [--verbose] -- queryMatrixProjects [--page=1] [--pageSize=15] [--projectName=] [--ids=]");
  console.error("  matrix -- queryMatrixProjectSprints [--page=1] [--pageSize=15] [--projectId=] [--sprintName=] [--statusCondition=]");
  console.error("  matrix -- queryMatrixItemsStatus");
  console.error("  matrix -- createMatrixProject --projectName=项目名称");
  console.error("  matrix -- createMatrixSprint --sprintName=迭代名称 --projectId=项目id [--beginTime=]");
  console.error("  matrix -- createMatrixTask --projectId=项目id或项目名 --taskName=任务名称 [--requirementId=] [--type=] [--sprintId=] [--priority=] [--description=] ...");
  console.error(
  ...
  console.error("  matrix -- queryMatrixTasks [--page=1] [--pageSize=10] [--processor=] [--creator=] [--rd=] [--qa=] ...");
  console.error(
  ...
  console.error("  matrix -- queryMatrixProjectRequirements --projectId=项目id [--page=1] [--pageSize=10] [--sIds=] [--requireName=] ...");
  process.exit(1);
}
