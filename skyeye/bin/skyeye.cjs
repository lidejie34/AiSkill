 ...
 * - 无 production dependencies 时，运行 skyeye.ts 仅需 Node（--experimental-strip-types），不强制安装。
...
function needsNpmInstall(root) {
  ...
  // console.log("skyeye dependencies", deps);
...
if (needsNpmInstall(pkgRoot)) {
  console.error("skyeye: 未安装依赖或依赖不完整，正在执行 npm install …");
  const install = spawnSync("npm", ["install"], { stdio: "inherit", cwd: pkgRoot });
  if (install.status !== 0) process.exit(install.status ?? 1);
}

const script = path.join(pkgRoot, "scripts", "skyeye.ts");
